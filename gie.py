...
file_phi = XDMFFile('droplet_ve/phigie.xdmf')
file_u = XDMFFile('droplet_ve/ugie.xdmf')
file_p = XDMFFile('droplet_ve/pgie.xdmf')
file_tau = XDMFFile('droplet_ve/taugie.xdmf')
...
rho_out = 1
Re_in = 100 #14.19436759
Re_out = 10 #1.419436759
De_in = 0
De_out = 25 #8.082903769
We_out = 400 #35.28
beta_out = 0.75 #0.07
...
eps = 0.01 #Constant((0.5*mesh.hmin()**0.9)/0.6)
...
def De(phi):
    ...
    return De_out #De_in*phi + De_out*(1-phi)
...
def eps1(phi):
    ...
    return eps_in*phi + eps_out*(1-phi)
...
T=9 #1*(np.sqrt(980*0.6))/0.6 #5.25
...
dt= 0.01 #((0.1/nx/2)*(np.sqrt(980*0.6))/0.6) #0.0404
num_steps = int(T/dt)
g = Constant((0, 1))
...
F_stress = (1/dt)*inner(De(phi)*(tau-tau0),S)*dx \
         ...
         + inner(De(phi)*(flux(u0,n,tau)('+') - flux(u0,n,tau)('-')),jump(S))*dS) \
         + 0.5*((inner(dot(De(phi)*u0,nabla_grad(tau0)) \
         - dot(De(phi)*tau0, trans(grad(u0))) \
         - dot(De(phi)*grad(u0), tau0),S)*dx \
         + inner(tau0, S)*dx) \
         - (1-beta(phi))*inner(grad(u0)+trans(grad(u0)),S)*dx \
         + inner(De(phi)*(flux(u0,n,tau0)('+') - flux(u0,n,tau0)('-')),jump(S))*dS) \
         + inner(alpha*(De(phi)/(1-beta(phi)))*dot(tau0,tau0), S)*dx \
...
ns1 = (1/dt)*inner((rho(phi)*u - rho(phi00)*u0), v)*dx \
    ...
    + (c(phi)/Re_out)*inner(tau0, grad(v))*dx \
...
#     plot(phi,cmap='jet')
#     plt.title('T = 0')
#     plt.savefig('/media/will/Samsung_T5/bubble_para/levelset_results/phi.png')
...
#     plt.subplot(122)
#     plot(p_)
#     plt.savefig('/media/will/Samsung_T5/bubble_para/levelset_results/u_p.png')


# print('Level set ic: ', assemble(sqrt (phiic.dx (0)**2 + phiic.dx (1)**2)*dx))
# print('Level set fin: ', assemble(sqrt (phi.dx (0)**2 + phi.dx (1)**2)*dx))