...
with open('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_20x80test_case1/Cons_Newtonian_CG_Dim_20x80test_case1_data.csv', newline='') as csvfile: #70
    datanr1601 = np.array(list(csv.reader(csvfile, delimiter='\t')))

with open('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_40x160test_case1/Cons_Newtonian_CG_Dim_40x160test_case1_data.csv', newline='') as csvfile: #70
    datanr1602 = np.array(list(csv.reader(csvfile, delimiter='\t')))

with open('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_80x320test_case1/Cons_Newtonian_CG_Dim_80x320test_case1_data.csv', newline='') as csvfile: #70
    datanr1603 = np.array(list(csv.reader(csvfile, delimiter='\t')))

with open('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_160x640test_case1/Cons_Newtonian_CG_Dim_160x640test_case1_data.csv', newline='') as csvfile: #70
...
for i in range(np.size(MooNMD_area)):
    fMooNMD_area.append(float(MooNMD_area[i])+0.2)
...
phi100 = Function(V)
with XDMFFile('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_20x80test_case1/phi_read.xdmf') as infile:
    infile.read_checkpoint(phi100, "phi")

mesh = RectangleMesh(Point(0.5,0),Point(1,2),40,160,'crossed')
V = FunctionSpace(mesh, 'CG', 2) 
phi200 = Function(V)
with XDMFFile('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_40x160test_case1/phi_read.xdmf') as infile:
    infile.read_checkpoint(phi200, "phi")

mesh = RectangleMesh(Point(0.5,0),Point(1,2),80,320,'crossed')
V = FunctionSpace(mesh, 'CG', 2) 
phi300 = Function(V)
with XDMFFile('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_80x320test_case1/phi_read.xdmf') as infile:
    infile.read_checkpoint(phi300, "phi")

mesh = RectangleMesh(Point(0.5,0),Point(1,2),160,640,'crossed')
V = FunctionSpace(mesh, 'CG', 2) 
phi400 = Function(V)
with XDMFFile('/media/wdoherty1/Samsung_T5/case1case2/Cons_Newtonian_CG_Dim_160x640test_case1/phi_read.xdmf') as infile:
...
# area1 = [abs(x-area1601[0])+area1601[0] for x in area1601]
# area2 = [abs(x-area1602[0])+area1602[0] for x in area1602]
# area3 = [abs(x-area1603[0])+area1603[0] for x in area1603]
# area4 = [abs(x-area1604[0])+area1604[0] for x in area1604]
...
# ax3.plot(fFreeLIFE_timescale,fFreeLIFE_area,color='blue',linestyle='-.')
# ax3.set_xlim([0,3])
# ax3.set_xlabel('t',**{'fontname':'Times', 'size':'18'})
# ax3.set_ylabel('Area Change',**{'fontname':'Times', 'size':'18'})
# ax3.grid()
# # ax3.set_title('Area')
# # ax3.set_yticks([0,0.00001,0.00002])
# ax3.legend(['M2','M3','M4','FreeLIFE'],loc='upper left',frameon=False)
...
ax7.legend(['M1','M2','M3','MooNMD','FreeLIFE'],loc='lower left',frameon=True)
...
ax_ins = inset_axes(ax7, width="100%", height="100%", loc='upper left',
                   bbox_to_anchor=(0.1,0.7,.4,.4), bbox_transform=ax7.transAxes)
...
ax_ins.set_xlim([0.55,0.60])
ax_ins.set_ylim([0.895, 0.92])
ax_ins.set_xticks([0.55,0.575,0.60])
ax_ins.set_yticks([0.90,0.91,0.92])
...
# ax_ins.tick_params(axis='y', colors='white')
# ax_ins.tick_params(axis='x', colors='white')
ax_ins.tick_params(axis='both', which='major', labelsize=14)

ax_ins2 = inset_axes(ax7, width="100%", height="100%", loc='upper right',
                   bbox_to_anchor=(0.63,0.3,.45,.45), bbox_transform=ax7.transAxes)
...
ax_ins2.set_xlim([0.8,0.85])
ax_ins2.set_ylim([0.95, 1.1])
ax_ins2.set_xticks([0.8,0.825,0.85])
...
# ax_ins2.set_yticks([0.96,1.10])
# ax_ins.tick_params(axis='y', colors='white')
# ax_ins.tick_params(axis='x', colors='white')
ax_ins2.tick_params(axis='both', which='major', labelsize=14)
